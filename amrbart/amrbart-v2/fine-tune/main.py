...
def main():
    ...
    data_collator = DataCollatorCate(
        ...
        pad_to_multiple_of=8 if training_args.fp16 else 1, # PY: why None?
    )
    
    metric = load_metric(path=str(pathlib.Path(__file__).parent / "metric/sacrebleu.py")) if training_args.task == "amr2text" else None
